...
<template>
    ...
    <LayoutDashboard>
        <div class="py-4">
            <div v-if="sourceTranslation" class="row">
                <div class="col-12">
            ...
            <div v-if="sourceTranslation" class="mt-4 overflow-hidden rounded-lg border bg-white shadow">
                <div class="relative z-10 flex w-full divide-x shadow">
                    <div class="flex w-14 shrink-0 items-center justify-center">
                        <div class="flex shrink-0 items-center">
                            <InputCheckbox :disabled="!translations.length" :checked="isAllSelected"
                    ...
                    <div class="flex h-full">
                        <div class="flex w-full max-w-full">
                            <button v-tooltip="selectedIds.length ? 'Delete selected' : 'Select languages to delete'"
                                type="button"
                    ...
                    <ConfirmationDialog size="sm" :show="showDialog">
                        <div class="flex flex-col p-6">
                            ...
                            <span class="mt-2 text-sm text-gray-500"> This action cannot be undone, This will
                                permanently delete the selected languages and all of their translations. </span>
                            ...
                            <div class="mt-4 flex gap-4">
                                ...
                                <BaseButton variant="danger" type="button" size="lg" :is-loading="loading" full-width
                                    @click="deleteTranslations"> Delete </BaseButton>
                            </div>
                        </div>
                    </ConfirmationDialog>
                </div>
                ...
                <div class="divide-y">
                    <div
                        class="flex h-14 flex-row gap-y-2 no-underline transition-colors duration-100 hover:bg-gray-50">
                        <div class="flex flex-1 divide-x border-r no-underline">
                            ...
                            <div
                                ...
                                <span class="mr-1 shrink-0 text-sm leading-tight text-gray-600">{{
                                    sourceTranslation.phrases_count }} source keys</span>
                            </div>
                            </Link>
                        </div>
                        ...
                        <Link v-tooltip="'Manage Keys'" :href="route('ltu.source_translation')"
                            class="group hidden w-full border-r sm:flex sm:w-14">
                        <div
                            class="relative inline-flex h-14 w-full cursor-pointer select-none items-center justify-center text-sm font-medium tracking-wide text-gray-700 outline-none transition-colors duration-150 ease-out focus:border-blue-50 group-hover:bg-blue-50 group-hover:text-blue-500">
                            <IconCog
                                class="hidden size-5 fill-current text-gray-400 group-hover:text-blue-500 sm:flex" />
                        </div>
                        </Link>
                        ...
                        <div v-tooltip="'Source language cannot be deleted!'" class="flex h-full">
                            <div class="flex w-full max-w-full">
                                <div
                                    class="relative inline-flex size-14 cursor-not-allowed select-none items-center justify-center text-sm font-medium uppercase tracking-wide text-gray-400 no-underline outline-none transition-colors duration-150 ease-out">
                                    <IconTrash class="size-5" />
                                </div>
                            </div>
                        </div>
                    ...
                    <template v-if="filteredTranslations.length">
                        <TranslationItem v-for="translation in filteredTranslations" :key="translation.language.code"
                            :translation="translation" :selected-ids="selectedIds" />
                    </template>

                    <Link v-if="filteredTranslations.length" :href="route('ltu.translation.create')"
                        class="flex cursor-pointer items-center justify-between rounded-b border-t p-4 text-gray-500 transition-colors duration-100 hover:bg-blue-50 hover:text-blue-600">
                    <div class="text-sm font-medium uppercase tracking-wide">Add new language</div>

                    <IconPlus class="size-6" />
                    </Link>

                    <EmptyState v-else class="bg-gray-50 py-32"
                        title="There are no languages to translate your project to."
                        description="Add the first one and start translating.">
                        <template #icon>
            ...
            <EmptyState v-else class="mt-4 min-h-[calc(100vh-11rem)] rounded-md border bg-white py-12 shadow"
                title="Source language has not been imported yet."
                description="Click the button below to start importing source translations. This process may take a few minutes to complete.">
                <template #icon>
                    <IconEmptyTranslations class=" w-20 text-gray-400" />
                </template>
